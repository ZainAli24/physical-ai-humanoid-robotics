...
Uses OpenAI GPT-4o-mini model for reliable function calling.
"""
...
def create_rag_agent() -> Agent:
    ...
    Environment Variables:
        ...
        - CHAT_MODEL: Model name (optional, defaults to gpt-4o-mini)

    Example:
        >>> agent = create_rag_agent()
        >>> # Use with Runner.run(agent, "What is ROS 2?")
    """
    # Get model name from env (optional - SDK uses sensible defaults)
    model_name = os.getenv("CHAT_MODEL", "gpt-4o-mini")
    print(f"[OK] Creating RAG agent with model: {model_name}")

    # Create agent - SDK handles OpenAI client automatically
    agent = Agent(
